...
public final class OptLabPreferences {
    ...
    private static final String K_REMOVED_FBO_FRAME_BUDGET = "fbo_frame_budget";
    private static final String K_REMOVED_STREAM_FBO_COORDINATOR = "stream_fbo_coordinator";
    ...
    public void setFeatureEnabled(@NonNull OptLabFeatureRegistry.Feature feature,
        ...
        commit(editor);
        requireStoredBoolean(feature.preferenceKey, value);
    }
    ...
    public void setGeneralProfile(@NonNull LabProfile profile) {
        ...
        commit(editor);
        requireStoredString(K_GENERAL_PROFILE, profile.name());
    }
    ...
    public void setBuild42LabProfile(@NonNull LabProfile profile) {
        ...
        commit(editor);
        requireStoredString(K_BUILD42_PROFILE, profile.name());
        if (profile != LabProfile.CUSTOM) {
            for (OptLabFeatureRegistry.Feature feature
                    : OptLabFeatureRegistry.Feature.values()) {
                if (feature.category != OptLabFeatureRegistry.Category.BUILD42
                ...
                requireStoredBoolean(feature.preferenceKey, expected);
            }
        }
    }
    ...
    public void setAdvancedControls(boolean enabled) {
        writeBoolean(K_ADVANCED_CONTROLS, enabled);
    }
    ...
    public void setProfile(Profile profile) {
        ...
        commit(editor);
        requireStoredString(K_PROFILE, profile.name());
    }
    ...
    public void setMasterEnabled(boolean value) {
        ...
        commit(prefs.edit().putBoolean(K_MASTER, true)
                ...
                .putString(K_GENERAL_PROFILE, LabProfile.CUSTOM.name()));
        requireStoredBoolean(K_MASTER, true);
    }

    public void setSafeModeEnabled(boolean value) {
        writeBoolean(K_SAFE_MODE, value);
    }
    ...
    public void setOnlyBuild42(boolean value) {
        writeBoolean(K_ONLY_BUILD42, value);
    }
    ...
    private void migrateR1508TrustRecovery() {
        ...
        SharedPreferences.Editor editor = prefs.edit()
                ...
                .remove(K_REMOVED_FBO_FRAME_BUDGET)
                .remove(K_REMOVED_STREAM_FBO_COORDINATOR)
                .putString(K_BUILD42_PROFILE, LabProfile.CUSTOM.name());
        for (OptLabFeatureRegistry.Feature feature : OptLabFeatureRegistry.Feature.values()) {
            if (feature.category == OptLabFeatureRegistry.Category.BUILD42
                    && feature.preferenceKey != null) {
    ...
    private void editBuild42(String key, boolean value) {
        commit(prefs.edit().putBoolean(key, value)
                .putString(K_BUILD42_PROFILE, LabProfile.CUSTOM.name()));
        requireStoredBoolean(key, value);
        requireStoredString(K_BUILD42_PROFILE, LabProfile.CUSTOM.name());
    }
    ...
    private void requireStoredBoolean(String key, boolean expected) {
        boolean actual = prefs.getBoolean(key, !expected);
        if (actual != expected) {
            throw new IllegalStateException("OPT-LAB setting read-back mismatch for " + key);
        }
    }

    private void requireStoredString(String key, String expected) {
        String actual = prefs.getString(key, null);
