    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding.gameSv.getHolder().addCallback(new SurfaceHolder.Callback() {
            ...
            public void surfaceChanged(@NonNull SurfaceHolder holder, int format, int width, int height) {
                ...
                OptLabPreferences optLab = OptLabPreferences.from(GameActivity.this);
                float refreshRate = 60.0f;
                if (getDisplay() != null && getDisplay().getMode() != null) {
                    refreshRate = getDisplay().getMode().getRefreshRate();
                }
                try {
                    // Surface may be published before GameLauncher.launch() begins, so establish
                    // its ownership policy before the JNI handoff.
                    Os.setenv("ZOMDROID_SURFACE_MODE", optLab.getSurfaceMode().name(), true);
                } catch (ErrnoException error) {
                    Log.w(LOG_TAG, "Unable to set Surface lifecycle policy", error);
                }
                if (optLab.getDisplayFpsHint()
                        == OptLabPreferences.DisplayFpsHint.NATIVE_REFRESH) {
                    try {
                        gameSurface.setFrameRate(refreshRate,
                                Surface.FRAME_RATE_COMPATIBILITY_FIXED_SOURCE);
                        Log.i("ZD-OPT-LAB", "displayFrameRateHint=" + refreshRate + " result=applied");
                    } catch (RuntimeException error) {
                        Log.w("ZD-OPT-LAB", "displayFrameRateHint=" + refreshRate + " result=failed", error);
                    }
                }
