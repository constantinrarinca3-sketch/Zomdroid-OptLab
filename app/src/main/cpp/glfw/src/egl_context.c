...
static void swapBuffersEGL(_GLFWwindow* window)
{
    ...
    if (g_zomdroid_surface.is_dirty) {
        ...
        const bool gen_ack = g_zomdroid_surface_gen_ack;
        pthread_mutex_unlock(&g_zomdroid_surface.mutex);

        ANativeWindow* previous_window = _glfw.zomdroid.aNativeWindow;
        if (gen_ack && window->context.egl.surface != EGL_NO_SURFACE) {
            // EGL resources must be detached and destroyed on their owning render thread.
            // Use the dynamically loaded EGL provider (MobileGL on the PZ route), never the
            // process-linked system libEGL entry points.
            _glfw.egl.MakeCurrent(_glfw.egl.display,
        ...
        if (!chooseEGLConfig(&ctxconfig, &fbconfig, &config)) {
            if (gen_ack && previous_window) ANativeWindow_release(previous_window);
            if (gen_ack) {
                pthread_mutex_lock(&g_zomdroid_surface.mutex);
...
GLFWbool _glfwInitEGL(void)
{
    ...
    if (_glfw_egl_library)
    {
        ...
        {
            _glfwInputError(GLFW_API_UNAVAILABLE,
                            "EGL: Failed to load requested library %s",
                            _glfw_egl_library);
            return GLFW_FALSE;
        }
        loadedName = _glfw_egl_library;
    }
    if (!_glfw.egl.handle)
    {
        for (i = 0;  sonames[i];  i++)
    ...
    _glfw.egl.prefix = (loadedName && strncmp(loadedName, "lib", 3) == 0);
                               ...
                               const _GLFWfbconfig* fbconfig)
{
    ...
    if (!_glfw.egl.KHR_get_all_proc_addresses)
    {
        ...
        if (_glfw_opengles_library)
        {
            ...
            {
                _glfwInputError(GLFW_API_UNAVAILABLE,
                                "EGL: Failed to load requested client library %s",
                                _glfw_opengles_library);
                return GLFW_FALSE;
            }
        }
        if (!window->context.egl.client)
