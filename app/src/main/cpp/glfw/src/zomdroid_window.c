                               ...
                               const _GLFWfbconfig* fbconfig)
{
    ...
    _GLFWctxconfig local;                  // EGL config used by GL4ES/NG/MobileGL
            ...
            || (g_zomdroid_renderer == MOBILEGL_PZCOMPAT))
    {
        ...
        // MobileGL must own both EGL and GLES entry points.  Its DirectGLES backend then opens
        // the Android driver internally.  Keeping both sides on the same wrapper is the working
        // P1 route and prevents a mixed MobileGL/system-EGL context.
        if (g_zomdroid_renderer == MOBILEGL_PZCOMPAT) {
            _glfw_egl_library = "libMobileGLPZ.so";
            _glfw_opengles_library = "libMobileGLPZ.so";
        }

        // (опционально) лог для проверки что env реально дошли
        __android_log_print(ANDROID_LOG_INFO, "ZomdroidGLFW",
                            "MGLPZ_GATE renderer=%d GLES=%d.%d EGL=%s GL=%s",
                            (int)g_zomdroid_renderer, local.major, local.minor,
                            _glfw_egl_library ? _glfw_egl_library : "system",
                            _glfw_opengles_library ? _glfw_opengles_library : "system");
...
void _glfwPollEventsZomdroid(void)
{
    if (g_zomdroid_input_mutex_safe) {
        ZomdroidEvent events[EVENT_QUEUE_MAX];
        ...
        unsigned int size = (head - tail) & EVENT_QUEUE_MAX;
        for (unsigned int i = 0; i < size; i++) {
            unsigned int next = (tail + i + 1U) & EVENT_QUEUE_MAX;
            events[i] = g_zomdroid_event_queue.buffer[next];
        }
        g_zomdroid_event_queue.safe_tail = head;
        pthread_mutex_unlock(&g_zomdroid_event_queue.mutex);

        for (unsigned int i = 0; i < size; i++)
            processZomdroidEvent(&events[i]);
        return;
    }
