...
public final class NativeModulesPreferencesUnit {
    public static void main(String[] args) {
        ...
        resumed.setLighting64Enabled(false);
        resumed.setPzClipperEnabled(false);
        resumed.setPathfindingEnabled(false);
        resumed.setPopManEnabled(false);
        NativeModulesPreferences allOff = NativeModulesPreferences.from(context);
        require(!allOff.isLighting64Enabled() && !allOff.isPzClipperEnabled()
                        && !allOff.isPathfindingEnabled() && !allOff.isPopManEnabled(),
                "one or more Native toggles re-armed after explicit OFF");
        ...
        System.out.println("NATIVE_MODULES_PREFERENCES_UNIT PASS schema=4 safe_mode=1"
                + " lighting_default_off=1 off_restart=all");
    }
