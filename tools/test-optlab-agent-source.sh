...
unzip -p "$WORK_DIR/bundle/zomdroid-agent.jar" com/zomdroid/agent/Main.class \
  > "$WORK_DIR/Main.class"
if grep -aEq 'FBO_FRAME_BUDGET|STREAM_FBO_COORDINATOR|fbo\.frame\.budget|stream\.fbo\.coordinator' \
    "$WORK_DIR/Main.class"; then
  echo 'removed FBO governor remains in bundled Main.class' >&2
  exit 1
fi
unzip -Z1 "$WORK_DIR/bundle/zomdroid-agent.jar" \
