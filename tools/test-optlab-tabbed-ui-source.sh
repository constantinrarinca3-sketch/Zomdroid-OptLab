...
assert 'build42_custom_presets_v1' in prefs and 'migrateR1508TrustRecovery' in prefs
...
for token in ('buildCustomPresetManagerPage', 'showCustomPresetManager',
              ...
              'OptLabCustomPresetStore', 'Preseturi custom Build 42'):
    assert token not in fragment, f'removed custom preset UI leaked into source: {token}'
...
with zipfile.ZipFile(io.BytesIO(payload)) as agent:
    ...
    assert b'[ZD-OPT-LAB-AGENT] version=16 schema=15 loaded' in \
