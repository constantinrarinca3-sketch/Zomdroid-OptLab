...
# Pathfinding can be the only enabled agent mechanism. It must still opt Byte Buddy into
# Java 25 class-file support before Project Zomboid classes are transformed.
...
grep -q '|| pathfindingNative.active;' \
  "$PROJECT_DIR/app/src/main/java/com/zomdroid/GameLauncher.java"
